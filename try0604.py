...
pt_zt = np.zeros([63,720])
sh_zt = np.zeros(720)
lh_zt = np.zeros(720)
t = np.zeros(720)
temp_zt = np.zeros([63,720])
qv_zt = np.zeros([63,720])
hei_th_zt = np.zeros([63,720])
pre_th_zt = np.zeros([63,720])
w_zt = np.zeros([63,720])
...
plt.figure(figsize=(7,4.5),dpi=80)
plt.plot(t/6.,sh_zt[:],linewidth=2,color='blue',label='sensible heaing')
plt.plot(t/6.,lh_zt[:],linewidth=2,color='red',label='latent heaing')
plt.xlim([0,120])
plt.xlabel('Time(hours)')
