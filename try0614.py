...
#dp_mean = np.zeros(nz)
dp = np.zeros([nz,nt])
#dqclpc2_zt = np.zeros([nz,nt])
#dqcfpc2_zt = np.zeros([nz,nt])
#dqpc2_zt = np.zeros([nz,nt])
#dqclconv_zt = np.zeros([nz,nt])
#dqcfconv_zt = np.zeros([nz,nt])
#dqconv_zt = np.zeros([nz,nt])
#dqcllsr_zt = np.zeros([nz,nt])
#dqcflsr_zt = np.zeros([nz,nt])
#dqlsr_zt = np.zeros([nz,nt])
#ppt_tot = np.zeros(nt)
#
for j in range(0,nt):
    t[j] = j
...
plt.figure(figsize=(7,4.5),dpi=80)
plt.plot(time,tot_pr_3h[:],linewidth=1,color='blue',label='precipitation')
plt.plot(t/144.,rc[:],linewidth=2,color='red',label='radiative cooling')
plt.xlim([0,50])
plt.xlabel('Time(days)')
plt.ylabel('Heating Rates(mm/day)')
plt.savefig('total precipitation rate(50d simulation)-1d mean.pdf')
plt.show()
